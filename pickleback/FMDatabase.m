...
- (void) compainAboutInUse:(NSString *)sql {
  //  //NSLog(@"The FMDatabase %@ is currently in use. The query was: %@", self, sql);
...
- (sqlite_int64) lastInsertRowId {
    ...
    if (inUse) {
        [self compainAboutInUse:@"lastInsertRowId"];
        return NO;
    }
    [self setInUse:YES];
